"""
chess_robot_ik_only_tcp_server.py — Pure IK chess robot with TCP server accept.

All servo angles are computed directly from inverse kinematics with
least-squares-fitted linear transforms (64-point physical calibration).

Base uses scale + offset (not just offset) to correct for mechanical play
that grows with angle. Shoulder and elbow use scale + offset as before.

Typical accuracy: ±2° on most squares, ±4° worst-case at board corners.
"""
import math
import socket
import time
...
import serial
from robot_tcp_server import TcpServerHelper
...
class ChessRobot:
    ...
    def __init__(
        ...
        if listen:
            try:
                self.tcp_server = TcpServerHelper(listen_host, tcp_port)
                self.transport = "tcp"
            except Exception as e:
                print(f"Eroare pornire server TCP: {e}")
                self.tcp_server = None
        elif host:
            try:
                ...
                self.transport = "tcp"
                print(f"Robot conectat TCP la {host}:{tcp_port}")
            except Exception as e:
                print(f"Eroare conexiune TCP: {e}")
                self.sock = None
        else:
            try:
    ...
    def send_command(self, cmd: str) -> str:
        if self.transport == "tcp":
            if self.tcp_server:
                return self.tcp_server.send_and_recv(cmd, timeout_sec=10)
            if self.sock is None:
                return "Nu sunt conectat!"
            try:
                self.sock.sendall(f"{cmd}\n".encode())
                return self._recv_line(timeout_sec=10)
            except Exception as e:
                try:
                    self.sock.close()
                except Exception:
    ...
    def _recv_line(self, timeout_sec: int = 5) -> str:
        if self.sock is None:
            return ""
    ...
    def send_angles_to_esp32(self, angles: Dict[ServoName, int]) -> bool:
        if self.transport == "tcp" and self.sock is None and self.tcp_server is None:
            print("Nu sunt conectat!")
            return False
        if self.transport == "serial" and self.serial is None:
            print("Nu sunt conectat!")
            return False
