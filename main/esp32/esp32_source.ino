...
// Wi-Fi STA settings (connect to RPi AP/router)
const char* WIFI_SSID = "chess_robot_free";
const char* WIFI_PASS = "";

// RPi TCP server settings
...
void scanWifiOnce() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        Serial.print(" dBm) ");
        Serial.println((WiFi.encryptionType(i) == WIFI_AUTH_OPEN) ? "OPEN" : "SEC");
    }
}
...
void loop() {
    ...
    if (!client || !client.connected()) {
        ...
        if (millis() - lastConnectAttemptMs >= CONNECT_RETRY_MS) {
            ...
            client.stop();
            if (client.connect(SERVER_HOST, TCP_PORT)) {
                Serial.println("TCP connected to RPi");
            } else {
    ...
    if (cmd.startsWith("B")) {
        ...
        client.print("OK:");
        client.print(base_angle);
        client.print(",");
        client.print(shoulder_angle);
        client.print(",");
        client.println(elbow_angle);
    }
}
